...
class tx_realurl extends tx_realurl_baseclass {
	...
	/**
	 * Actual host name (configuration key) for the current request.
	 * This can
	 ...
	 * This is to simplify identification of a single
	 ...
	 * (note: preVars only!).
	 * This is necessary for _DOMAINS feature. This value
	...
	/**
	 * Contains URL parameters that were merged into URL.
	 * This is necessary
	...
	/**
	 * Indicates wether cHash should be rebuilt for the URL.
	 * Used during
	 * encoding only.
	 *
	 * @var boolean
	 ...
	 *
	 * @param
	 *        	array Array of parameters from t3lib_tstemplate::linkData - the function creating all links inside TYPO3
	 * @return void
	 */
	...
	/**
	 * Prepends URL generated by RealURL by something (e.g.
	 * a host).
	 * This method gets called by the typoLink_PostProc hook in tslib_content:
	 *
	 * @param array $parameters
	 *        	Array of parameters from typoLink_PostProc hook in tslib_content
	 * @param tslib_cObj|\TYPO3\CMS\Frontend\ContentObject\ContentObjectRenderer $pObj
	 *        	Reference to the calling tslib_content instance
	 * @return void
	 */
	 ...
	 *        	Current URLs GETvar => value pairs in array, being translated into pathParts: Here we take out "id" GET var.
	 * @param array $pathParts
	 *        	Numerical array of path-parts, continously being filled. Here, the "page path" is being added by which-ever method is preferred. Passed by reference.
	 ...
	 *
	 * @param array $paramKeyValues
	 *        	Current URLs GETvar => value pairs in array, being translated into pathParts, continously shortend. Passed by reference.
	 * @param array $pathParts
	 *        	Numerical array of path-parts, continously being filled. Passed by reference.
	 * @param array $postVarSetCfg
	 *        	config
	 ...
	 *        	Keyword to set as a representation of the GETvars configured.
	 * @param array $keyValues
	 *        	Array of GETvar => values which content in $paramKeyvalues must match exactly in order to be substituted with the keyword, $keyWord
	 * @param array $paramKeyValues
	 *        	Current URLs GETvar => value pairs in array, being translated into pathParts, continously shortend. Passed by reference.
	 * @param array $pathParts
	 *        	Numerical array of path-parts, continously being filled. Passed by reference.
	 ...
	 *        	Host + the original URL with GET parameters - identifying the cached version to find
	 * @param array $internalExtras
	 *        	Array with extra data to include in encoding. This is flags if adminJump url or feLogin flags are set since these are NOT a part of the URL to encode and therefore are needed for the hash to be true.
	 * @param string $setEncodedURL
	 *        	If set, this URL will be cached as the encoded version of $urlToEncode. Otherwise the function will look for and return the cached version of $urlToEncode
	...
	protected function encodeSpURL_encodeCache($urlData, $internalExtras, $setEncodedURL = '') {
		...
		if (! $setEncodedURL) { // Asking for cached encoded URL:
			...
			if (! isset ( $GLOBALS ['TSFE']->applicationData ['tx_realurl'] ['_CACHE'] [$hash] ) && $this->extConf ['init'] ['enableUrlEncodeCache']) {
				 ...
				 * @noinspection PhpUndefinedMethodInspection
				 */
				$res = $GLOBALS ['TYPO3_DB']->exec_SELECTquery ( 'content', 'tx_realurl_urlencodecache', 'url_hash=' . $GLOBALS ['TYPO3_DB']->fullQuoteStr ( $hash, 'tx_realurl_urlencodecache' ) . ' AND tstamp>' . strtotime ( 'midnight', time () - 24 * 3600 * $this->encodeCacheTTL ) );
				/**
				 * @noinspection PhpUndefinedMethodInspection
				 */
	 ...
	 *
	 * @param string $newUrl
	 *        	URL path (being hashed to an integer and cHash value related to this.)
	 * @param array $paramKeyValues
	 *        	Params $array array, passed by reference. If "cHash" is the only value left it will be put in the cache table and the value is unset in the array.
	 * @return void
	 ...
	 *
	 * @param string $speakingURIpath
	 *        	Path from SpeakingURL.
	 * @return void
	...
	protected function decodeSpURL_checkRedirects($speakingURIpath) {
		 ...
		 * @noinspection PhpUndefinedMethodInspection
		 */
		list ( $redirectRow ) = $GLOBALS ['TYPO3_DB']->exec_SELECTgetRows ( 'destination,has_moved,domain_limit', 'tx_realurl_redirects', 'url_hash=' . $hash . ' AND url=' . $url . ' AND domain_limit IN (0,' . $domainId . ')', '', 'domain_limit DESC' );
		if (is_array ( $redirectRow )) {
			 ...
			 * @noinspection PhpUndefinedMethodInspection
			 */
			$GLOBALS ['TYPO3_DB']->exec_UPDATEquery ( 'tx_realurl_redirects', 'url_hash=' . $hash . ' AND url=' . $url . ' AND domain_limit=' . $redirectRow ['domain_limit'], $fields_values, array (
					'counter' 
			) );
	...
	protected function getCurrentDomainId() {
		 ...
		 * @noinspection PhpUndefinedMethodInspection
		 */
		list ( $row ) = $GLOBALS ['TYPO3_DB']->exec_SELECTgetRows ( 'uid', 'sys_domain', 'domainName=' . $GLOBALS ['TYPO3_DB']->fullQuoteStr ( $this->apiWrapper->getIndpEnv ( 'HTTP_HOST' ), 'sys_domain' ) . ' AND redirectTo=\'\'' );
	 ...
	 *
	 * @param string $speakingURIpath
	 *        	Speaking URL path (after the "root" path of the website!) but without query parameters
	 * @param boolean $cHashCache
	 *        	If cHash caching is enabled or not.
	 ...
	 *        	Array to generate strings from
	 * @param string $prependString
	 *        	path to prepend to every parameter
	 ...
	 *
	 * @param array $pathParts
	 *        	Parts of path. NOTICE: Passed by reference.
	 ...
	 *
	 * @param array $pathParts
	 *        	The path splitted by "/". NOTICE: Passed by reference and shortend for each time a segment is matching configuration
	 * @param array $config
	 *        	Configuration
	...
	/**
	 * Fix for the magic_quotes_gpc.
	 * See http://bugs.typo3.org/view.php?id=18133
	 *
	 * @param mixed $array        	
	 * @return void
	 */
	 ...
	 *
	 * @param array $pathParts
	 *        	Path parts of the URLs (can be modified)
	 ...
	 * @param array $pathParts
	 *        	Parts of path. NOTICE: Passed by reference.
	 * @param array $setupArr
	 *        	Setup array for segments in set.
	 ...
	 *
	 * @param string $speakingURIpath
	 *        	Speaking URL path to be decoded
	 * @param array $cachedInfo
	 *        	Optional; If supplied array then this array is stored as the cached information for the input $speakingURIpath. If this argument is not set the method tries to look up such an array associated with input speakingURIpath
	...
	protected function decodeSpURL_decodeCache($speakingURIpath, array $cachedInfo = NULL) {
		if ($this->extConf ['init'] ['enableUrlDecodeCache'] && ! $this->disableDecodeCache) {
			...
			if (is_array ( $cachedInfo )) { // STORE cachedInfo
				 ...
				 * @noinspection PhpUndefinedMethodInspection
				 */
				$res = $GLOBALS ['TYPO3_DB']->exec_SELECTquery ( 'content', 'tx_realurl_urldecodecache', 'url_hash=' . $GLOBALS ['TYPO3_DB']->fullQuoteStr ( $hash, 'tx_realurl_urldecodecache' ) . ' AND ' . 
	...
	/**
	 * Get "cHash" GET var from database.
	 * See explanation in encodeSpURL_cHashProcessing()
	 *
	 * @param string $speakingURIpath
	 *        	Speaking URL path (virtual path)
	...
	protected function decodeSpURL_cHashCache($speakingURIpath) {
		 ...
		 * @noinspection PhpUndefinedMethodInspection
		 */
		list ( $row ) = $GLOBALS ['TYPO3_DB']->exec_SELECTgetRows ( 'chash_string', 'tx_realurl_chashcache', 'spurl_hash=' . $GLOBALS ['TYPO3_DB']->fullQuoteStr ( md5 ( $stringForHash ), 'tx_realurl_chashcache' ) );
	...
	/**
	 * Doing database lookup between "alias values" and "id numbers".
	 * Translation is bi-directional.
	 *
	 * @param array $cfg
	 *        	Configuration of look-up table, field names etc.
	 * @param string $value
	 *        	Value to match field in database to.
	 * @param boolean $aliasToUid
	 *        	If TRUE, the input $value is an alias-string that needs translation to an ID integer. FALSE (default) means the reverse direction
	...
	protected function lookUpTranslation($cfg, $value, $aliasToUid = FALSE) {
		...
		if ($aliasToUid) {
			...
			if ($cfg ['useUniqueCache'] && $returnId = $this->lookUp_uniqAliasToId ( $cfg, $value )) {
				 ...
				 * @noinspection PhpUndefinedMethodInspection
				 */
				$res = $GLOBALS ['TYPO3_DB']->exec_SELECTquery ( implode ( ',', $fieldList ), $cfg ['table'], $cfg ['alias_field'] . '=' . $GLOBALS ['TYPO3_DB']->fullQuoteStr ( $value, $cfg ['table'] ) . ' ' . $cfg ['addWhereClause'] );
				/**
				 * @noinspection PhpUndefinedMethodInspection
				 */
				$row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc ( $res );
			...
			if ($cfg ['useUniqueCache'] && ! $cfg ['autoUpdate'] && $returnAlias = $this->lookUp_idToUniqAlias ( $cfg, $value, $lang )) {
				 ...
				 * @noinspection PhpUndefinedMethodInspection
				 */
				$res = $GLOBALS ['TYPO3_DB']->exec_SELECTquery ( implode ( ',', $fieldList ), $cfg ['table'], $cfg ['id_field'] . '=' . $GLOBALS ['TYPO3_DB']->fullQuoteStr ( $value, $cfg ['table'] ) . ' ' . $cfg ['addWhereClause'] );
				/**
				 * @noinspection PhpUndefinedMethodInspection
				 */
				$row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc ( $res );
				...
				if ($row) {
					...
					if ($langEnabled && $lang) {
						 ...
						 * @noinspection PhpUndefinedMethodInspection
						 */
						$res = $GLOBALS ['TYPO3_DB']->exec_SELECTquery ( $cfg ['alias_field'], $cfg ['table'], $cfg ['transOrigPointerField'] . '=' . intval ( $row ['uid'] ) . '
	 ...
	 *        	Alias value to convert to ID
	 * @param boolean $onlyNonExpired
	 *        	<code>true</code> if only non-expiring record should be looked up
	 ...
	 *        	ID value to convert to alias value
	 * @param integer $lang
	 *        	sys_language_uid to use for lookup
	 * @param string $aliasValue
	 *        	Optional alias value to limit search to
	 ...
	 *
	 * @param array $cfg
	 *        	Configuration array of lookup table
	 * @param string $newAliasValue
	 *        	Preferred new alias (final alias might be different if duplicates were found in the cache)
	 * @param integer $idValue
	 *        	ID associated with alias
	 * @param int $lang
	 *        	sys_language_uid to store with record
	 ...
	 *        	Main key in realurl configuration array. Default is "postVarSets" but could be "fixedPostVars"
	...
	public function getPostVarSetConfig($pageId, $mainCat = 'postVarSets') {
		...
		$cfg = is_array ( $this->extConf [$mainCat] [$pageId] ) ? $this->extConf [$mainCat] [$pageId] : (is_array ( $this->extConf [$mainCat] ['_DEFAULT'] ) ? $this->extConf [$mainCat] ['_DEFAULT'] : array ());
	...
	protected function getLocalizedPostVarSet($postVarSet = array()) {
		...
		if ($this->currentMode !== self::MODE_ENCODE || ! is_array ( $postVarSet ) || empty ( $postVarSet )) {
			return $postVarSet;
		}
	...
	/**
	 * Rawurlencodes the input string; used for GET parameter names of variables that were NOT SpURL encoded.
	 * Offers the possibility of NOT encoding them...
	 *
	 * @param string $str
	 *        	Input string
	 ...
	 *
	 * @param array $setup
	 *        	Configuration for condition
	 * @param string $prevVal
	 *        	Previous value in sequence of GET vars. The value is the "system" value; In other words: The *real* id, not the alias for a value.
	 ...
	 *
	 * @param string $type
	 *        	Calling type of realurl (encode|decode)
	 * @param array $params
	 *        	Parameters delivered to RealURL (e.g. from t3lib_TStemplate->linkData hook)
	 ...
	 *        	Configuration required to determine hosts while path encoding
	 * @param array $params
	 *        	Parameters delivered to RealURL by t3lib_TStemplate->linkData hook
	 ...
	 *
	 * @param array $configuration
	 *        	Configuration required to determine hosts while path decoding
	 * @return void
	 */
	 ...
	 *
	 * @param string $useConfiguration
	 *        	Reference to another existing configuration part which shall be used
	...
	public function clearPageCacheMgm($params) {
		$pageIdArray = $params ['table'] == 'pages' ? array (
				intval ( $params ['uid'] ) 
		) : $params ['pageIdArray'];
		...
		if (! $this->enableStrictMode) {
			...
			if (! $rootpage_id && ! $this->multidomain) {
				...
				if (count ( $rows ) == 1) {
					...
					$this->devLog ( 'Found rootpage_id by searching sys_template', array (
							'rootpage_id' => $rootpage_id 
					) );
				}
			}
	...
	protected function adjustRootPageId() {
		if (! $this->extConf ['pagePath'] ['rootpage_id']) {
			...
			if ($this->enableStrictMode) {
				if ($this->pObj) {
					$this->pObj->pageNotFoundAndExit ( 'RealURL strict mode error: ' . 'multidomain configuration without rootpage_id. ' . 'Please, fix your RealURL configuration!' );
				} else {
					throw new Exception ( 'RealURL error: ' . 'unable to determine rootpage_id for the current domain.' );
				}
			}
			...
			if ($this->multidomain && ! $this->extConf ['pagePath'] ['rootpage_id']) {
				if ($this->pObj) {
					$this->pObj->pageNotFoundAndExit ( 'RealURL error: ' . 'unable to determine rootpage_id for the current domain.' );
				} else {
					throw new Exception ( 'RealURL error: ' . 'unable to determine rootpage_id for the current domain.' );
				}
			}
	...
	/**
	 * Returns the detected language (decoding only).
